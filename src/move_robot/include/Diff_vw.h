...
private:
	...
	float nTemp_Send_V, nTemp_Send_W;

	float Rev_x, Rev_y;
	float Rev_roll, Rev_pitch;
	float acc_y, acc_z;
	int Ground_Object_Id, CheckPoint_Id;

	float Rev_acc_x, Rev_acc_y, Rev_acc_z, Rev_gyr_x, Rev_gyr_y, Rev_gyr_z;
	int type;
	...
	std::vector<float> vRevInfo_last;
	...
	bool Check_EV_Pose;
	float V_odm_l, V_odm_r;
	float dis_odm_l; // sampling time = 0.1 sec
	float dis_odm_r; // sampling time = 0.1 sec
	float dis_odm_avg;
	float odm_x;
	float odm_y;
	float last_odm_x;
...
diff_vw::diff_vw(char *dev_name, int Baudrate) : Move_Robot(dev_name, Baudrate)
{
	...
	IMU_Encoder_Publisher_ = node_.advertise<sensor_msgs::Imu>("IMU_data", 20);
}
diff_vw::~diff_vw()
{
}
		...
		if (now_index < First_tracking)
		{
			...
			if (fabs(cmd_angular_velocity) >= move_w_max)
			{
				if (cmd_angular_velocity > 0)
					cmd_angular_velocity = move_w_max;
				else
					cmd_angular_velocity = -1.0 * move_w_max;
			}
	...
	for (int i = 0; i < i_obs_container.size(); i++)
	{
		...
		if (p.x() < protect_OBS_Front && p.x() > -1 * Car_Front)
		{
			if (fabs(p.y()) < OBS_SideMiss_Scale * protect_OBS_Side)
			{
				...
				if (p.y() < -1 * protect_OBS_Side && p.x() < Car_Front)
					i_obs_region_4.push_back(p);
				else if (p.y() < -1 * protect_OBS_Side && p.x() >= Car_Front)
						   ...
						   std::vector<Eigen::Vector3f> &all_point)
{
	...
	if (gap_array.size() > 0)
	{
		...
		if (max_dis >= gap_limit)
		{
			...
			if (warn_theta > (2 * M_PI))
				warn_theta = warn_theta - 2 * M_PI;

			if (warn_theta > M_PI)
				warn_theta = 2 * M_PI - warn_theta;
...
void diff_vw::Calculate_odom()
{
	...
	std::cout << "Rev_V = " << Rev_V << " "
			  << " Rev_W =  " << Rev_W << std::endl;
...
void diff_vw::RevProcess(double receive_period)
{
	...
	while (1)
	{
		...
		if (mySerial.serial_ok == true)
		{
			...
			// 	packet_size = 44;
			readByte = read(mySerial.fd, buff, packet_size);

			// std::cout << "RRR " << buff[0]<<" " << buff[1]<<" "<<buff[readByte-2]<<" "<< buff[readByte-1]<< std::endl;
				...
				else
				{
					if (packet_size == 29)
					{
						if (buff[1] == 'C' && buff[readByte - 2] == 'C')
						{
							...
							// vPacket.reserve(packet_count); //改變vector預分配空間
							...
							Rev_y = vPacket[3];
							Rev_th = vPacket[4];
							std::cout << "Rev_V: " << Rev_V << std::endl;
							std::cout << "Rev_W: " << Rev_W << std::endl;
							std::cout << "Rev_th: " << Rev_th << std::endl;
							if (vRevInfo_last.size() != 0)
							{
								geometry_msgs::PoseStamped pose_msg;
								pose_msg.header.frame_id = "Robot_Pose";
								pose_msg.header.stamp = ros::Time::now();
								pose_msg.pose.position.x = vPacket[2];
								pose_msg.pose.position.y = vPacket[3];
								pose_msg.pose.position.z = vPacket[4];

								pose_msg.pose.orientation.x = vRevInfo_last[0] - vPacket[2];
								pose_msg.pose.orientation.y = vRevInfo_last[1] - vPacket[3];
								pose_msg.pose.orientation.z = vRevInfo_last[2] - vPacket[4];
								AGV_Pose_Publisher_.publish(pose_msg);
							}
							else
							{
								vRevInfo_last.reserve(3);
								vRevInfo_last.push_back(vPacket[2]);
								vRevInfo_last.push_back(vPacket[3]);
								vRevInfo_last.push_back(vPacket[4]);
							}

							vRevInfo_last[0] = vPacket[2];
							vRevInfo_last[1] = vPacket[3];
							vRevInfo_last[2] = vPacket[4];
						}
						else
						{
							rev_buf.clear();
					...
					else
					{
						if (buff[1] == 'I' && buff[readByte - 2] == 'I')
						{
							...
							if (vRevInfo_last.size() != 0)
							{
								...
								imu_data.header.frame_id = "Robot_Pose_Imu";
								imu_data.orientation.x = Rev_V;
								imu_data.orientation.y = Rev_W;
								imu_data.orientation.z = nTemp_Send_V;
								imu_data.orientation.w = nTemp_Send_W;
								imu_data.angular_velocity.y = Rev_pitch;
								imu_data.angular_velocity.z = Rev_th;
								imu_data.linear_acceleration.y = acc_y;
								imu_data.linear_acceleration.z = acc_z;

								AGV_Imu_Publisher_.publish(imu_data);
							}
							else
							{
								vRevInfo_last.reserve(3);
			...
			// 	{
			// 		recv_str.push_back(rev_buf[i]);
			// 	}
			// std::stringstream cut(recv_str);

			// while (getline(cut, sub_str, ','))
			// {
			// 	recv_pkg[recv_cnt] = sub_str;
			// 	recv_cnt++;
			// }
			...
			// 	Rev_th = std::stod(recv_pkg[4]);
			// 	// Rev_acc_x = std::stof(recv_pkg[5]);
			// 	// Rev_acc_y = std::stof(recv_pkg[6]);
			// 	// Rev_acc_z = std::stof(recv_pkg[7]);
			// 	// Rev_gyr_x = std::stof(recv_pkg[8]);
			// 	// Rev_gyr_y = std::stof(recv_pkg[9]);
			// 	// Rev_gyr_z = std::stof(recv_pkg[10]);

			// 	// std::cout << "Rev_V: " << Rev_V << " Rev_W: " << Rev_W << " Rev_x: " << Rev_x << " Rev_y: " << Rev_y << " Rev_th: " << Rev_th << std::endl;
			// 	// std::cout << "msg_acc_x: " << Rev_acc_x << " msg_acc_y: " << Rev_acc_y << " msg_acc_z: " << Rev_acc_z << std::endl;
			// 	// std::cout << "msg_gyr_x: " << Rev_gyr_x << " msg_gyr_y: " << Rev_gyr_y << " msg_gyr_z: " << Rev_gyr_z << std::endl;
			...
			// 	// // msg.orientation.w = 0;
			// 	// //Acc
			// 	// msg.linear_acceleration.x = Rev_acc_x;
			// 	// msg.linear_acceleration.y = Rev_acc_y;
			// 	// msg.linear_acceleration.z = Rev_acc_z;
			// 	// //Gyro
			// 	// msg.angular_velocity.x = Rev_gyr_x;
			// 	// msg.angular_velocity.y = Rev_gyr_y;
			// 	// msg.angular_velocity.z = Rev_gyr_z;
			// 	// IMU_Encoder_Publisher_.publish(msg);
			// }
			// else if (rev_buf[0] == 'E' && rev_buf[1] == 'I' && rev_buf[rev_buf.size() - 4] == 'I' && rev_buf[rev_buf.size() - 3] == 'E')
			// {
			// 	std::string recv_str;
			// 	std::string sub_str;
			// 	std::string recv_pkg[6];
			// 	int recv_cnt = 0;
			// 	for (int i = 3; i < rev_buf.size() - 5; i++)
			// 	{
			// 		recv_str.push_back(rev_buf[i]);
			...
			// 	sensor_msgs::Imu msg;
			// 	msg.header.frame_id = "base_link";
			// 	msg.linear_acceleration.x = std::stof(recv_pkg[0]);
			// 	msg.linear_acceleration.y = std::stof(recv_pkg[1]);
			// 	msg.linear_acceleration.z = std::stof(recv_pkg[2]);
			// 	msg.angular_velocity.x = std::stof(recv_pkg[3]);
			// 	msg.angular_velocity.y = std::stof(recv_pkg[4]);
			// 	msg.angular_velocity.z = std::stof(recv_pkg[5]);
			// 	IMU_Encoder_Publisher_.publish(msg);
			// }
			rev_buf.clear();
		}
...
void diff_vw::RevPacketDecorder(std::vector<unsigned char> &buff, unsigned int bufferSize, std::vector<float> &vPacket)
{
	...
	for (int i = 0; i < bufferSize; ++i)
	{
		...
		if (buff[i * 5 + 2] > 0) //vaiue is <0
			packet_value *= -1;
		// std::cout << "packet_value : " << i << " Buffer size: " << bufferSize << " Value:" << packet_value << std::endl;
